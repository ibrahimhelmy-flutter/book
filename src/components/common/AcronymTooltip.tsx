...
export function AcronymTooltip({
  ...
  return (
    ...
    >
      ...
      {/* Floating Tooltip displaying just full English name (rendered only when visible to prevent GPU layer bloat) */}
      {isVisible && (
        <span
          role="tooltip"
          className="absolute bottom-full left-1/2 -translate-x-1/2 mb-2 px-3 py-1.5 bg-slate-900 border border-sky-500/40 rounded-xl shadow-xl z-50 text-left dir-ltr whitespace-nowrap max-w-[85vw] pointer-events-none block animate-fadeIn"
        >
          {/* Tooltip Arrow */}
