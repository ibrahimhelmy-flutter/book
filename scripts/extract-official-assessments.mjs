...
// 14 Lessons Definitions matching Official Textbook and Assessment Booklet
const LESSON_DEFS = [
...
export function fixArabicLigatures(text) {
  if (!text) return '';
  return text
    .replace(/\bامل/g, 'الم')
...
export function parseOptionsRobust(rawLines) {
  const combined = rawLines.join(" ");

  // Case 1: Inverted format with bracket: e.g. "200 )أ 404 )ب"
  if (/([^\s()]+)\s*[)）]\s*[اأب]/.test(combined)) {
    const opts = ['', '', '', ''];
    const invRegex = /([^\s()]+(?:\s+[^\s()]+)*?)\s*[)）]\s*([اأبجد])/g;
    let m;
  ...
  if (/([^\s\-–]+)\s*[-–]\s*[اأب]/.test(combined)) {
    const opts = ['', '', '', ''];
    const invDashRegex = /([^\s\-–]+(?:\s+[^\s\-–]+)*?)\s*[-–]\s*([اأبجد])/g;
    let m;
  ...
  const optRegex = /(?:^|\s+)(?:[([（]?([اأبجد])[\s)\]）\-–.]|([اأبجد])[\s)\]）\-–.])\s*([\s\S]*?)(?=(?:\s+(?:[([（]?[اأبجد][\s)\]）\-–.]|[اأبجد][\s)\]）\-–.]))|$)/g;
  const matches = [];
  let match;
...
export function extractRawQuestionsForLesson(def) {
  const linesWithMeta = [];
  ...
  for (let i = 0; i < linesWithMeta.length; i++) {
    ...
    if (qMatch) {
      ...
      let inlineOpts = [];
      const inlineOptMatch = qBody.match(/(?:^|\s+)(?:[([（]?([اأ])[\s)\]）\-–.]|([اأ])[\s)\]）\-–.])/);
      ...
      const inlineInvMatch = qBody.match(/(?:^|\s+)(?:[^\s\-–]+)\s*[-–]\s*[اأ]/);
    ...
    if (currentQuestion) {
      if (currentQuestion.type === "mcq") {
        const isOptionToken = /(?:^|\s+)[اأبجد]\s*[-–.)]|(?:[^\s()]+)\s*[)）]\s*[اأبجد]|(?:[^\s\-–]+)\s*[-–]\s*[اأبجد]|(?:^|\s+)[(（][اأبجد][)）]/.test(text) ||
