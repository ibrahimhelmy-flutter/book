...
for (const chNum of chapterNumbers) {
  ...
  // If a modular chapter directory exists with lesson-*.json files or unit-*-review.json, merge and generate modular TS files
  if (fs.existsSync(chSubDir) && fs.statSync(chSubDir).isDirectory()) {
    const lessonFiles = fs.readdirSync(chSubDir).filter(f => /^(lesson-\d+-\d+|unit-\d+-review)\.json$/.test(f)).sort();
    ...
    for (const lFile of lessonFiles) {
      ...
      const lessonMatch = lFile.match(/^lesson-(\d+)-(\d+)\.json$/);
      const unitReviewMatch = lFile.match(/^unit-(\d+)-review\.json$/);

      if (lessonMatch) {
        const lCh = lessonMatch[1];
        const lIdx = lessonMatch[2];
        lKey = `${lCh}-${lIdx}`;
        constName = `LESSON_${lCh}_${lIdx}_DEEP_QUESTIONS`;
        tsFileName = `lesson-${lCh}-${lIdx}`;
      } else if (unitReviewMatch) {
        const uCh = unitReviewMatch[1];
        lKey = `${uCh}-review`;
        constName = `UNIT_${uCh}_REVIEW_DEEP_QUESTIONS`;
        tsFileName = `unit-${uCh}-review`;
      } else {
        continue;
      }
 ...
 * Source of Truth: book-sources/term-1/05-canonical-data/authored/deep-questions/chapter-${chNum}/${lFile}
 * Generated by: scripts/build-all-data.mjs (Deterministic Reproducible Build)
 */
...
`;
      safeWriteFileSync(path.join(targetChDir, `${tsFileName}.ts`), lessonTsContent, 'utf-8');
      lessonImports.push(`import { ${constName} } from "./${tsFileName}";`);
      lessonExports.push(`export * from "./${tsFileName}";`);
    ...
    for (const [key, qList] of Object.entries(questions)) {
      if (!lessonFiles.includes(`lesson-${key}.json`) && !lessonFiles.includes(`unit-${key}.json`)) {
...
export function getDeepQuestionsForLesson(lessonIdentifier: string | { id: string; number: string; title?: string }): DeepChallengingQuestion[] {
  ...
  while (result.length < 50) {
    ...
    result.push({
      ...
      correctAnswer: 0,
      answer: "الموازنة الدقيقة بين الأداء الهندسي العالي، وموثوقية الأمان، وقابلية الصيانة والتوسع على المدى الطويل",
